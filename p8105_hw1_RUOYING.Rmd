...
as.numeric(factor)
```
Logical variable can be convert to numerical variable, where TRUE convert to 1, and FALSE convert to 0. So the mean can be calculated. Factor vector will be assigned a numeric value to each factor level based on the order of appearance.
For character variable, it contains non-numeric characters, so it cannot be convert to numerical variable, then the mean cannot be calculated.
